...
def pipeline(args):
    ...
    basins = df.groupby(
            ...
            )["basin"].unique().apply(lambda x: x[0])
    basins = basins.apply(
            lambda x: "colorado" if x in ["upper_col", "lower_col"] else x
    )
    basin_counts = basins.value_counts()
    idx = pd.IndexSlice
    ...
    cmp = pd.DataFrame({"simmed":simmed, "preds":preds_act})
    cmp = cmp.dropna()
    diff = cmp["preds"] - cmp["simmed"]
    diff = diff.abs().groupby(diff.index.get_level_values(0)).mean()
...
def parse_args(arg_list=None):
    ...
    parser.add_argument(
        ...
        default=44,
        type=int,
        help="Number to seed random generator with for reservoir selection"
    )
    if arg_list:
