...
def scaled_MixedEffects(df, groups, filter_groups=None, scaler="mine"):
    ...
    res_sets = X_scaled.groupby("compositegroup").apply(lambda x: x.index.get_level_values(1).unique())
    sizes = res_sets.apply(lambda x: x.size)
    lvout_ratio = 0.5
    lvout_size = (sizes * lvout_ratio).apply(lambda x: int(np.floor(x)))
    lvout_res_sets = []
    np.random.seed(33)
    for i in range(10):
        lvout = []
        for g,n in lvout_size.items():
            lvout.extend(np.random.choice(res_sets[g], n, replace=False))
        lvout_res_sets.append(lvout)
    ...
    lvout_results = {}
    idx = pd.IndexSlice
    lvout_res_sets = [[i] for i in X_scaled.index.get_level_values(1).unique()]
    for test_res in lvout_res_sets:
        X_test = X_scaled.loc[idx[:,test_res],:]
        train_res = list(filter(lambda x: x not in test_res, X_scaled.index.get_level_values(1).unique()))
        X_train = X_scaled.loc[idx[:,train_res],:]
    ...
    # Instead of adding a constant, I want to create dummy variable that accounts for season differences
        ...
        exog_re.loc[:,"Storage_pre_act"] = df.loc[test_index, "Storage_pre"]
        forecasted = forecast_mixedLM_new(coefs, exog_re, means.loc[test_res], std.loc[test_res], "compositegroup", actual_inflow_test).dropna()
        ...
        forc_score = r2_score(y_test_act.loc[forecasted_act.index], 
                                forecasted_act)
        forc_rmse = mean_squared_error(y_test_act.loc[forecasted_act.index], forecasted_act, squared=False)
        ...
        fc_res_scores = pd.DataFrame(index=test_res, columns=["NSE", "RMSE"])
        for res in test_res:
            y = y_test_act.loc[forecasted_act.index,res]
            f = forecasted_act[res]
            fc_res_scores.loc[res, "NSE"] = r2_score(y, f)
            fc_res_scores.loc[res, "RMSE"] = mean_squared_error(y, f, squared=False)
        ...
        output = dict(
            coefs=coefs,
            fc_res_scores=fc_res_scores,
            fit_score=f_act_score,
            fit_rmse=f_act_rmse,
            fit_bias=f_bias,
            fit_bias_month=f_bias_month,
            pred_score=p_act_score,
            pred_rmse=p_act_rmse,
            forc_score=forc_score,
            forc_rmse=forc_rmse,
            data=dict(
            #     X_test=X_test,
            #     y_test=y_test,
        ...
        lvout_results[test_res[0]] = (output)
    
    output_dir = pathlib.Path(f"../results/agu_2021_runs/simple_model_spatial_validation_all_res_lvoneout")
    if not output_dir.exists():
        output_dir.mkdir(parents=True)

    with open(output_dir / "results.pickle", "wb") as f:
        pickle.dump(lvout_results, f, protocol=4)
