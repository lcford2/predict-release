...
from simple_model import forecast_mixedLM, predict_mixedLM
...
def tree_model(X, y, tree_type="decision", **tree_args):
    if tree_type == "decision":
        ...
        def_args = dict(max_depth=4, min_samples_leaf=100, bootstrap=True, n_jobs=-1,)
        for key, value in tree_args.items():
            def_args[key] = value
        tree = RandomForestRegressor(**def_args)
    else:
        raise ValueError(
            "Invalid tree_type provided. Must be either 'Decision' or 'Extra'."
        )
...
def get_leaves_and_groups(X, tree):
    ...
    if leaves.ndim == 2:
        groups = pd.DataFrame(
            leaves, columns=range(1, leaves.shape[1] + 1), index=X.index
        )
    else:
...
# @time_function
def sub_tree_multi_level_model(
    X, y, tree=None, groups=None, my_id=None, drop_rel_roll: bool = False
):
    if tree:
        if drop_rel_roll and "release_roll7" in X.columns:
            leaves, groups = get_leaves_and_groups(
                X.drop(["release_roll7", *calendar.month_abbr[1:]], axis=1), tree
            )
        else:
            leaves, groups = get_leaves_and_groups(
                X.drop(calendar.month_abbr[1:], axis=1), tree
            )
    elif not groups:
    ...
    free = MixedLMParams.from_components(
        fe_params=np.ones(mexog.shape[1]), cov_re=np.eye(X.shape[1])
    )
    try:
...
def predict_from_sub_tree_model(
    ...
    if forecast:
        ...
        preds = predict_mixedLM(
            ml_model.params, ml_model.random_effects, mexog, exog_re, "group"
        )
...
def pipeline(month_intercepts=False):
    ...
    ml_model = sub_tree_multi_level_model(
        X_train, y_train_rel, tree, drop_rel_roll=True
    )

    coefs = pd.DataFrame(ml_model.random_effects)
    fitted = ml_model.fittedvalues
    y_train_rel_act = (
        y_train_rel.unstack().T * std.loc[train_res, "release"]
    ...
    f_bias_month = (
        fitted_act.groupby(time_grouper.month).mean()
    ...
    export_graphviz(
        tree,
        out_file=(folderpath / "tree.dot").as_posix(),
