...
def pipeline(args):
    ...
    X_test_act["sto_diff"] = X_test_act["storage_pre"] - X_test_act["storage_roll7"]
    ...
    coefs = pd.DataFrame.from_records(
        [ror_params, lrt_params, hrt_params],
        columns=X_vars, index=["ROR", "LRT", "HRT"]
    )

    ror_train = pd.DataFrame({"release": ror_train, "group": "ror"})
    lrt_train = pd.DataFrame({"release": lrt_train, "group": "lrt"})
    hrt_train = pd.DataFrame({"release": hrt_train, "group": "hrt"})

    ror_test = pd.DataFrame({"release": ror_test, "group": "ror"})
    lrt_test = pd.DataFrame({"release": lrt_test, "group": "lrt"})
    hrt_test = pd.DataFrame({"release": hrt_test, "group": "hrt"})

    fitted = pd.concat([ror_train, lrt_train, hrt_train]).sort_index()
    preds = pd.concat([ror_test, lrt_test, hrt_test]).sort_index()

    simuled = simulate_three_model(
        coefs,
        X_test_act,
        means,
        std,
    ...
    fitted_act = (
        fitted["release"].unstack().T * std.loc[train_res, "release"]
        + means.loc[train_res, "release"]
    ).T.stack()
    preds_act = (
        preds["release"].unstack().T * std.loc[test_res, "release"] + means.loc[test_res, "release"]
    ).T.stack()
...
# print(simmed_res_scores.to_markdown(floatfmt="0.3f"))
    ...
    foldername = f"stovars_three_model{assim_mod}"
    folderpath = pathlib.Path("..", "results", "three_reg_model", basin, foldername)
...
def simul_reservoir(
    ...
    for date in dates:
        ...
        reg_vars_no_sd = ["storage_pre", "release_pre", "inflow",
                          "storage_roll7", "release_roll7", "inflow_roll7"]
        X_r = rdf.loc[idx[res, date], reg_vars_no_sd]
        ...
        reg_vars_no_sd += ["storage_x_inflow"]
        X_r = (X_r - means.loc[res, reg_vars_no_sd]) / std.loc[res, reg_vars_no_sd]
        X_r["sto_diff"] = X_r["storage_pre"] - X_r["storage_roll7"]
        # and the constant
        X_r["const"] = 1
        if res_vars:
            X_r["max_sto"] = max_sto
        ...
        group = preds.loc[idx[res, date], "group"]
        release = X_r[["const"] + reg_vars] @ model.loc[group.upper()]
