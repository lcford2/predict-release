...
tva_nid = gpd.tools.sjoin(nid_gdf, huc_buffer, how="inner", op="within")

min_size = 1
max_size = 600
min_value = tva_nid["Max_Storage"].min()
max_value = tva_nid["Max_Storage"].max()
print(tva_nid["Max_Storage"].sum())

ratio = (max_size - min_size) / (max_value - min_value)
sizes = tva_nid["Max_Storage"] * ratio + min_size
...
tva_nid.plot(ax=ax, markersize=sizes)
